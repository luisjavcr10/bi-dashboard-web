...
import { getReporteProduccionMerma } from "@/lib/queries/reportes";
...
export async function GET(req: NextRequest) {
    try {
        ...
        const data = await getReporteProduccionMerma(filters);
        return NextResponse.json({ status: "ok", data });
    } catch (error) {
        console.error("Error fetching Produccion Merma report:", error);
